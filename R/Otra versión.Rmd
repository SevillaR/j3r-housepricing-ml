...
xgb.model <- xgb.train(param, data = train_Data, watchlist, nrounds = 10000,
                           maximize = FALSE,
...
predict.Test <- predict(xgb.model, Test_data)

submission <- data.frame(Id=Test$Id, SalePrice=predict.Test)

write.csv(submission, file = 'Xgboost_model.csv', row.names = F)
