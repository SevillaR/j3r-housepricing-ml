...
```{r,message=FALSE, warning=FALSE}
# Split the data into Training and Test Set # Ratio: 6:4 ### # Cambio a 0.8 (antes 0.6)
Training_Inner<- Training[1:floor(length(Training[,1])*0.8),]
...
xgb.model <- xgb.train(param, data = train_Data, watchlist, nrounds = 20000,
                           maximize = FALSE,
                           early_stopping_rounds = 5,
                           print_every_n = 100)
best.iter <- xgb.model$best_iteration

importance_matrix <- xgb.importance(colnames(train_Data), model = xgb.model)
...
prediction <- predict(xgb.model, test_data)
rmse(log(Test_Inner$SalePrice),log(prediction))
```


All the transformation done to Train now to Test:
...
Test.matrix <- as.matrix(Test, rownames.force=NA)
Test_data <- xgb.DMatrix(data = Test.matrix[,2:76])

train.submis <- as.matrix(Training, rownames.force=NA)
train.submis <- as(train.submis, "sparseMatrix")
# Never forget to exclude objective variable in 'data option'
train_Data.submis <- xgb.DMatrix(data = train.submis[,2:76], label = train.submis[,"SalePrice"])
...
xgb.model.submis <- xgb.train(param, data = train_Data.submis, nrounds = best.iter,
                           print_every_n = 50)


importance_matrix.submis <- xgb.importance(colnames(train_Data.submis), model = xgb.model.submis)

xgb.plot.importance(importance_matrix.submis[1:20,], rel_to_first = TRUE, xlab = "Relative importance")


predict.Test <- predict(xgb.model.submis, Test_data)

submission <- data.frame(Id=Test$Id, SalePrice=predict.Test)

write.csv(submission, file = 'Xgboost_model Training.all.csv', row.names = F)
